...
ThreadInfo add_to_prio_queue(PriorityQueue *prio_queue, tid_t thread_id,
			unsigned int priority, tid_t curr_thread_id)
{
	LinkedList *list;

	list = prio_queue->priority_lists[priority];
	return add_to_list(list, thread_id, curr_thread_id);
}

void append_node_to_queue(PriorityQueue *prio_queue,
				LinkedListNode *node, unsigned int priority)
{
	append_node(prio_queue->priority_lists[priority], node);
}

int delete_from_prio_queue(PriorityQueue *prio_queue,
				tid_t thread_id, unsigned int priority)
{
	return delete_from_list(prio_queue->priority_lists[priority], thread_id);
}

LinkedListNode *remove_from_prio_queue(PriorityQueue *prio_queue,
				tid_t thread_id, unsigned int priority)
{
