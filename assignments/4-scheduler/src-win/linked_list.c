...
ThreadInfo add_to_list(LinkedList *list, tid_t thread_id, tid_t curr_thread_id)
{
	...
	node->thread_info.sem = CreateSemaphore(
		NULL,
		0,
		1,
		NULL
	...
	if (node->thread_info.sem == INVALID_HANDLE_VALUE) {
		invalid_thread_info.id = INVALID_TID;
		return invalid_thread_info;
	}
	...
	if (list->size == 0) {
		...
		/* daca coada listei are id-ul curr_thread_id */
		/* elementul se pune pe antepenultima pozitie */
		curr_tid = curr_thread_id;
		end_tid = list->end->thread_info.id;
		if (curr_tid != UNUSED_CURR_THREAD && end_tid == curr_tid) {
			...
			/* se pune dupa antepenultimul element */
			/* si inainte de ultimul element */
			curr_node->next = node;
			node->next = list->end;
			if (list->size == 1)
				list->start = node;
...
int free_node(LinkedListNode *node)
{
	...
	bRet = CloseHandle(node->thread_info.sem);
	if (bRet == FALSE)
		return FAILURE;
